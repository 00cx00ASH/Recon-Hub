// scan-path-traversal — injeta payloads de path traversal (../../etc/passwd
// com variantes de encoding/bypass) em parâmetros que carregam caminho de
// arquivo e confirma LFI/traversal só quando a ASSINATURA de um arquivo de
// sistema conhecido (/etc/passwd: "root:x:0:0"; win.ini: "[fonts]") aparece
// na resposta com o payload mas NÃO no baseline sem ele. Só lê arquivo de
// sistema como PoC — nunca escreve, nunca sai do "eu provei que dá ler".
...
// embedded fallback — usado só se nenhuma wordlist for resolvida. Nomes
// de parâmetro que costumam carregar caminho/nome de arquivo (onde path
// traversal/LFI vive), não os parâmetros genéricos de id/busca.
var builtinParams = []string{
	"file", "filename", "filepath", "path", "page", "template", "tpl",
	"include", "inc", "document", "doc", "download", "dir", "folder",
	"load", "read", "view", "content", "item", "data", "resource", "url",
	"name", "style", "lang", "locale", "theme", "layout", "module", "conf",
	"config", "settings", "log", "logfile", "img", "image", "attachment",
}
...
func main() {
	...
	worker := func() {
		...
		for t := range ch {
			...
			for _, pay := range probePayloads {
				u := withParamRaw(t.base, t.param, pay)
				if u == "" {
					continue
				}
				...
				emit(ev{
					...
					Title:    "Path traversal / LFI em " + t.param + " (" + hostOf(t.base) + ")",
					Asset:    u,
					Evidence: fmt.Sprintf("param %q, payload %q → assinatura de arquivo de sistema (%q) na resposta, ausente no baseline sem o payload — leitura de arquivo arbitrário confirmada", t.param, pay, sig),
	...
	emit(ev{Type: "done", OK: true, Msg: fmt.Sprintf("%d URL(s), %d requisições, %d path traversal/LFI", len(bases), reqs, hit)})
}
...
func fetch(c *http.Client, u string) (string, error) {
	...
	req.Header.Set("User-Agent", "recon-hub/scan-path-traversal")
	applyAuth(req)
	resp, err := c.Do(req)
	if err != nil {
		return "", err
...
// withParamRaw substitui o valor de `key` pelo payload EXATO, sem re-encodar
// (os demais params seguem encodados normalmente). Traversal depende de
// controle fino de encoding — um `%2e%2e%2f` pré-encodado não pode virar
// `%252e...` por um Encode() automático, e um `../` cru tem que chegar como
// `../`. url.URL.String() emite RawQuery verbatim, então montamos a query à
// mão: os outros params via Encode(), o nosso via key=payload literal.
func withParamRaw(u, key, payload string) string {
