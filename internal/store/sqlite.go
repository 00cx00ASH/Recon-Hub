...
CREATE TABLE IF NOT EXISTS findings (
  ...
  triage TEXT, triaged_at TEXT, triage_reason TEXT
);
...
func OpenSQLite(path string) (*SQLiteStore, error) {
	...
	_, _ = db.Exec(`ALTER TABLE findings ADD COLUMN triage_reason TEXT`)
...
func (s *SQLiteStore) AddFinding(f *Finding) (bool, error) {
	...
	_, ierr := s.db.Exec(`INSERT INTO findings
	 (id,dedup_key,job_id,tool,program,target,type,severity,title,asset,evidence,meta,count,created_at,last_seen,triage,triaged_at,triage_reason)
	 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		f.ID, key, f.JobID, f.Tool, f.Program, f.Target, f.Type, f.Severity, f.Title, f.Asset,
		f.Evidence, string(f.Meta), f.Count, tstr(f.CreatedAt), tstr(f.LastSeen), f.Triage, tptr(f.TriagedAt), f.TriageReason)
	return true, ierr
}

func (s *SQLiteStore) ListFindings(f FindingFilter) ([]*Finding, error) {
	q := `SELECT id,job_id,tool,program,target,type,severity,title,asset,evidence,meta,count,created_at,last_seen,triage,triaged_at,triage_reason FROM findings`
	where, args := whereClause(map[string]string{
	...
	for rows.Next() {
		...
		var triage, triagedAt, triageReason sql.NullString
		if err := rows.Scan(&fd.ID, &fd.JobID, &fd.Tool, &fd.Program, &fd.Target, &fd.Type, &fd.Severity,
			&fd.Title, &fd.Asset, &fd.Evidence, &meta, &fd.Count, &created, &last, &triage, &triagedAt, &triageReason); err != nil {
			return nil, err
		}
		if meta != "" {
		...
		fd.TriageReason = triageReason.String
		out = append(out, &fd)
	}
	return out, rows.Err()
}
...
func (s *SQLiteStore) GetFinding(id string) (*Finding, bool) {
	row := s.db.QueryRow(`SELECT id,job_id,tool,program,target,type,severity,title,asset,evidence,meta,count,created_at,last_seen,triage,triaged_at,triage_reason FROM findings WHERE id=?`, id)
	var fd Finding
	var meta, created, last string
	var triage, triagedAt, triageReason sql.NullString
...
func (s *SQLiteStore) SetFindingTriage(id, verdict, reason string) (*Finding, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	now := time.Now().UTC()
	res, err := s.db.Exec(`UPDATE findings SET triage=?, triaged_at=?, triage_reason=? WHERE id=?`, verdict, tstr(now), reason, id)
	if err != nil {
		return nil, err
	}
