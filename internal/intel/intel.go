...
var knownRisk = map[string]risk{
	...
	"stored-xss":                        {0.95, "confirmado por EXECUÇÃO real num navegador (o onerror da <img> persistida disparou numa navegação SEPARADA, depois do submit), não por heurística de texto — praticamente zero falso positivo. É mais grave que reflected/DOM XSS: o payload fica ARMAZENADO e executa pra QUEM abrir a página — se meta.cross_session=true, foi confirmado executando numa sessão diferente da que submeteu (ex: admin vendo um comentário de atacante = account takeover de privilégio alto sem interação). Confira CSP/HttpOnly no cookie de sessão pra calibrar o impacto no relatório, mas a correção é escapar/sanitizar na SAÍDA (onde o valor é renderizado), não só na entrada. Cite o submit_url (onde entra) e o view_url (onde dispara)."},
}
